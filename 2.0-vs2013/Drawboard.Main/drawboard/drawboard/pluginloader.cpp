...
vector<string> &PluginLoader::getPluginNames() {
  ...
  for (int i = 0; i < m_HModList.size(); i++) {
    ...
    PLUGIN_PROC_NAME getPluginName = (PLUGIN_PROC_NAME)GetProcAddress(hLibMod, "getPluginName");
...
vector<QIcon *> &PluginLoader::getPluginIcons() {
  for (int i = 0; i < m_HModList.size(); i++) {
    HMODULE hLibMod = m_HModList.at(i);
    PLUGIN_PROC_ICON getPluginIcon = (PLUGIN_PROC_ICON)GetProcAddress(hLibMod, "getPluginIcon");
...
QList<HMODULE> &PluginLoader::getDllModList() { return m_HModList; }
