...
void Arc::drag(QMouseEvent *evt, QPaintDevice *device) {
    // 取得坐标增量
...
void Arc::addKeyPoints(QMouseEvent *evt) {
    if (keyPoints.size() <= 1) {
...
void Arc::addKeyPoints(QPoint &point) {
    keyPoints.push_back(point);
}

void Arc::clearKeyPoints() {
    keyPoints.clear();
}

void Arc::setP1(QPoint &point) {
    this->p1 = new QPoint(point);
}

void Arc::setP2(QPoint &point) {
